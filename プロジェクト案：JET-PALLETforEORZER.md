...
### アビリティ
JET-PALLETでの判定や攻撃にあたる。アビリティによっては使用しない欄もあるため、未入力の場合は項目名語と省略して出力する。（たとえば黒魔道士のトランスは「名前」「タグ」「基本効果」のみ。）
**ユーザーがスキルの内容を記録する役割しか持たないものは「ラベル」として説明を割愛します。**
- `種別`: "メイン", "サブ", "インスタント","特殊"から選択する。種別によって配置できる領域が変わる。「特殊」以外の選択した種別はタグにも含まれる。
- `アイコン`：ラベル。
- `アビリティ名`: ユーザーが自由に入力する。アビリティの名称。
- `スタック数`：使用可能な回数。使うと１減る。0や未入力の場合は回数制限なし。ゲーム上はしばしば制限という形で表現されるが、数値として扱うために別のラベルとする。
- `タグ`：ユーザーが自由に入力する。今後タグのキーワードを対象に処理を行う可能性がある。アビリティの基本効果、バフの効果説明のテキストに登録されたタグと一致する文字列は、cardの表示時に斜体にする。"物理","魔法","連射","メイン","サブ","風属性",etc.
- `前提`：ラベル
- `タイミング`：ラベル
- `コスト`: ラベル
- `制限`：ラベル
- `対象`：ラベル
- `範囲`：ラベル
- `判定`："d20+{使用するステータス} アビリティ名"という形でBCdiceに使用できるコマンドにする。ステータスを使用しない場合は"+{使用するステータス}"は省略。
- `基本ダメージ`：アビリティが基本的に出力するダメージロール。
- `ダイレクトヒット`：「ダイレクトヒット」がONのときに追加するダメージロール。
- `基本効果`：ラベル

アビリティはツールチップとしてcardを表示する。これはルールブックに準拠した表記で見やすくするため。

#### マクロ
アビリティの使用条件の設定や、使用されたときに実行する行動などを設定する。ビジュアルプログラミングのような操作感で設定できるようにする。現時点では複雑な階層化は不要と思われる。
- `前提条件`：アビリティを使用するために必要な条件。満たしていない場合警告を表示する。
- `アクション`：アビリティを使用したときに実行する行動。MPの消費などもここで設定する。
**マクロブロック**
マクロを構成する単位。ドラッグで並び替えも可能。上から順に処理される。
- `条件`：数値で扱っているデータ（バフの数、リソース、アビリティのスタック数）を対象に比較する。対象はユーザー登録データから動的に変わる。同じグループ内で結んだ条件は連言として扱う。
...
#### マクロ仕様の補足（実装判断の前提）
- 条件グループ内の接続子は `AND/OR` を任意に混在させる。
- 条件グループ間は上から順に評価し、並び順が実行順序を決める。
- 条件ブロックの直後に置かれたアクションのみ、その条件を満たした場合に実行する。以降のアクションは独立して実行される。
- 条件/アクションの対象に含まれる「アビリティ」はスタック数を指す。
- 「判定・ダメージを追加」「判定・ダメージを変更」は、ユーザーがBCDice文法に沿ったコマンド文字列を入力する方式とする。
- 「効果テキストを追加」は、`[ロールコマンド][修正値等][半角スペース][アビリティ名][半角スペース][バフによる追加テキスト][半角スペース][アクションで追加されるテキスト]` の末尾に連結する。
- 前提条件を満たしていなくてもクリックは可能で、警告トーストに不足条件を表示する。
- マクロはアビリティごとに1つ設定する。
...
- **評価の単位**
  ...
  - 条件グループ間は `groupConnectors` で `AND/OR` を指定できる。
- **実行対象**
  - `buff/resource/ability` は数値として評価し、`change` は数値であれば上限/下限に丸めて更新する。
  - `add-judge-damage` と `change`（文字列値）は判定・ダメージコマンドを文字列として更新する。
  - `show-choice` は選択肢ごとにネストしたアクションを実行する。

#### UI操作フローとデータ保存・復元（簡潔版）
- **マクロ編集フロー**
  1. アビリティ編集モーダルからマクロ設定モーダルを開く。
  2. 条件/アクションブロックを追加・並び替えし、プレビューで確認する。
  3. 「アビリティに反映」を押下すると、マクロJSONをアビリティのデータに反映する。
- **保存**
  - マクロはJSON文字列としてアビリティ要素の `data-macro` に保存し、アビリティデータの永続化に含める。
  - 全体の保存はローカルストレージへJSONで保存する。
- **復元**
  - 初期ロード時にローカルストレージからアビリティデータを読み込み、`data-macro` を復元する。
  - モーダルを開いた際、`data-macro` の内容を編集用状態へ読み込む。
...
#### バフの表示
- バフエリアにはアクティブなバフのみが表示される。場合によって、適宜登録済みバフ一覧から手動でもバフエリアに追加できる。
- バフに「ターン終了まで」の継続時間が設定されている場合、バフの下部に「0t」と表示される。「次のターン開始まで」の場合は「1t」、永続の場合はそのまま。
- バフにコマンドへ影響する効果が設定されていたとしても、ツールチップに表示するのは「名前」「種別」「詳細」のみです。これは、アビリティを含め、表面上の記述を文語的な者のみにして、それをどのような形でゲーム中に再現するかはユーザーによるところが大きいためです。
...
モックアップの時点ではpngのイメージ画像を置いているが、実際にはsvgファイルでカラーの設定ができるようにしたい。
...
### 各種UI
- `ターン開始ボタン`：「1t」のバフを削除する。
- `ターン終了ボタン`：「0t」のバフを削除する。
- `ラウンド終了ボタン`：MPの現在値を2上昇させる。
- `フェイズ更新ボタン`：すべてのバフを削除する。
- `ダイレクトヒット`：アビリティに設定されたダイレクトヒットの効果をダメージロールに加える。
- `クリティカル`：ダメージロールのダイスロール部分(2d6など)のダイス数（2d6なら2の部分）をすべて2倍にする。例）「2d6+1d6+2+1」のダメージロールがクリティカルの場合、「4d6+2d6+2+1」になる。
- `-/+ボタン`：各リソースの現在値を1ずつ上昇・減少させる。
...
## 備考
- 「特殊」のアビリティはメイン、サブ、インスタントのいずれからも独立した種別で、例えば吟遊詩人の詩や、ユーザーがオリジナルでアビリティを設定したいときなどに使用します。
...
- ターン終了ボタンやラウンド終了ボタン等は、単純に対応したバフの削除や、MPの回復などの機能をつけたボタンにする予定だが、このボタンもマクロを設定できるようにして、特定のバフ（主にアストラルファイア）があるとMPを回復しないなどができるようにしたほうがいいかも。実装方法は要検討。
- proc-lineは条件成立中に強調表示として使用する。使用条件だけでなく、条件成立時に追加のアクションを実行するアビリティの条件が成立している際にも表示する。（崩拳の参の功力など）
- 「ダイレクトヒット」「クリティカル」「プロック」等については、実際にダイスロールを行ってから決まる要素なので、手動で解決する。
- 画像をソフト内に完全にアップロードすると、ブラウザのストレージ制限（5MB）にすぐ引っかかってしまうので、セーブデータには画像のファイル名のみ保存するのがよかろうと思う。プロジェクト内に画像ファイルを置いておけるフォルダを用意して、アイコンはそこから選ぶ形にする。ツール内でアップ的なことができないのはストレスなので、アップされたファイルはその画像ファイル置き場に格納する？
- -これなら大量の画像をあらかじめ入れておきたいとき、エクスプローラーなどからフォルダに直接入れられる。また、同じ構造のフォルダとファイルを他のユーザーと共有していれば、バフの一括追加記法をコピーしたときに画像ファイル名も文字列に含めれば、登録時に一致するファイル名があればそれを使用する、みたいな形で即時に同じアイコンのバフを扱える。
