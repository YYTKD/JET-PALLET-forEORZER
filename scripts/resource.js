...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const createResourceAdminControls = () => {
        const listRoot = document.querySelector("[data-trait-resource-list]");
        if (!listRoot) {
            return null;
        }

        const nameInput = document.querySelector("[data-resource-name]");
        const currentInput = document.querySelector("[data-resource-current]");
        const maxInput = document.querySelector("[data-resource-max]");
        const styleSelect = document.querySelector("[data-resource-style]");
        const colorSelect = document.querySelector("[data-resource-color]");
        const submitButton = document.querySelector("[data-resource-submit]");
        ...
        const setFormValues = (resource) => {
            resourceAdminState.editingId = resource?.id ?? null;
            nameInput.value = resource?.name ?? "";
            currentInput.value =
                resource?.current !== undefined ? String(resource.current) : "";
            maxInput.value = resource?.max !== undefined ? String(resource.max) : "";
            styleSelect.selectedIndex =
                resource?.style === RESOURCE_STYLES.stack ? 1 : 0;
            if (resource?.color) {
                colorSelect.value = resource.color;
            } else {
        ...
        const renderResourceAdminList = () => {
            ...
            injectSvgIcons(listRoot);
        };

        submitButton.addEventListener("click", () => {
            const id =
                resourceAdminState.editingId ?? `resource-${Date.now()}`;
            const name = nameInput.value.trim();
            const current = readNumberInput(currentInput, RESOURCE_DEFAULTS.min);
            const max = readNumberInput(maxInput, RESOURCE_DEFAULTS.max);
            const style =
    ...
    const resourceAdminControls = createResourceAdminControls();
    if (resourceAdminControls) {
        resourceAdminControls.setFormValues(null);
        resourceAdminControls.renderResourceAdminList();
    }
